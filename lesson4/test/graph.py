...
def polyline(points):
  ...
  return line
#----------------------------------
def rectangle(x1, y1, x2, y2, x0 = 0, y0 = 0, fi = 0):
  fi *= math.pi / 180
  
  xN = (x1 - x0) * math.cos(fi) - (y1 - y0) * math.sin(fi)
  yN = (x1 - x0) * math.sin(fi) + (y1 - y0) * math.cos(fi)
  x1 = x0 + xN
  y1 = y0 + yN
  
  xN = (x2 - x0) * math.cos(fi) - (y2 - y0) * math.sin(fi)
  yN = (x2 - x0) * math.sin(fi) + (y2 - y0) * math.cos(fi)
  x2 = x0 + xN
  y2 = y0 + yN  
